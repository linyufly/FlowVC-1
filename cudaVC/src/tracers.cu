...
void GenerateStaggeredRelease(void) {
	...
	if (Memory_Usage_Tracer == Use_Pinned_Memory) {		
			...
			err = cudaHostAlloc( (void**)&Tracer1.x, N_Total * sizeof(double ), cudaHostAllocDefault );
			if(err != cudaSuccess) {
				fprintf(stderr, "Something went terribly wrong in allocating Host Memory for Tracer1.x .\n");
				printf("CUDA Error: %s \n\n", cudaGetErrorString(err));
				exit(1);
			}
		
			// y-position
			err = cudaHostAlloc( (void**)&Tracer1.y, N_Total * sizeof(double ), cudaHostAllocDefault );
			if(err != cudaSuccess) {
				fprintf(stderr, "Something went terribly wrong in allocating Host Memory for Tracer1.y .\n");
				printf("CUDA Error: %s \n\n", cudaGetErrorString(err));
				exit(1);
			}
				...
				// z-position
				err = cudaHostAlloc( (void**)&Tracer1.z, N_Total * sizeof(double ), cudaHostAllocDefault );
				if(err != cudaSuccess) {
					fprintf(stderr, "Something went terribly wrong in allocating Host Memory for Tracer1.z .\n");
					printf("CUDA Error: %s \n\n", cudaGetErrorString(err));
					exit(1);
				}
		
			// ElementIndex
			err = cudaHostAlloc( (void**)&Tracer1.ElementIndex, N_Total * sizeof(int ), cudaHostAllocDefault );
			if(err != cudaSuccess) {
				fprintf(stderr, "Something went terribly wrong in allocating Host Memory for Tracer1.ElementIndex .\n");
				printf("CUDA Error: %s \n\n", cudaGetErrorString(err));
				exit(1);
			}
		
			// LeftDomain
			err = cudaHostAlloc( (void**)&Tracer1.LeftDomain, N_Total * sizeof(int ), cudaHostAllocDefault );
			if(err != cudaSuccess) {
				fprintf(stderr, "Something went terribly wrong in allocating Host Memory for Tracer1.LeftDomain .\n");
				printf("CUDA Error: %s \n\n", cudaGetErrorString(err));
				exit(1);
			}
		
			// Start Time
			err = cudaHostAlloc( (void**)&Tracer1.Start_time, N_Total * sizeof(double ), cudaHostAllocDefault );
			if(err != cudaSuccess) {
				fprintf(stderr, "Something went terribly wrong in allocating Host Memory for Tracer1.Start_time .\n");
				printf("CUDA Error: %s \n\n", cudaGetErrorString(err));
				exit(1);
			}
		
			// Stop time
			err = cudaHostAlloc( (void**)&Tracer1.Stop_time, N_Total * sizeof(double ), cudaHostAllocDefault );
			if(err != cudaSuccess) {
				fprintf(stderr, "Something went terribly wrong in allocating Host Memory for Tracer1.Stop_time .\n");
				printf("CUDA Error: %s \n\n", cudaGetErrorString(err));
				exit(1);
			}
			
			// index
			err = cudaHostAlloc( (void**)&index1, N_Total * sizeof(int ), cudaHostAllocDefault );
			if(err != cudaSuccess) {
				fprintf(stderr, "Something went terribly wrong in allocating Host Memory for index .\n");
				printf("CUDA Error: %s \n\n", cudaGetErrorString(err));
				exit(1);
			}
			
			// index
			err = cudaHostAlloc( (void**)&Tracer.Status, N_Total * sizeof(int ), cudaHostAllocDefault );
			if(err != cudaSuccess) {
				fprintf(stderr, "Something went terribly wrong in allocating Host Memory for Tracer1.Status .\n");
				printf("CUDA Error: %s \n\n", cudaGetErrorString(err));
				exit(1);
			}
	...
	if ( Trace_ReleaseStrategy == 0 ) {
		...
		if (Generate_Frame) {
			...
			if(Data_MeshType == UNSTRUCTURED) {
				...
				if (Searching == 1)
					//globalsearch_device();
					FatalError("Searching on Device is demoted. \n\n");
				else { // Searching on Host
					search_host();
...
int Output_Tracer(float tmin, float tmax, int Output_Release) {
