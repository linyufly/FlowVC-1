...
double pEuler(LagrangianPoint *pt, double tstart, double tend) {
	...
	double h, tc, dx, X0[3], vec1[3], vec2[3], u0[3], dudt0[3], dotu0[3], gradu0[3][3], wp[3], strain_rate0 = 0, strain_rate1 = 0;
	int ii, jj;
	...
	while((tc + TINY) < tend) { /* Integration not complete */
		...
		if(Vel_MeshElementFlagArray[pt->ElementIndex] && Int_NormalFlow) 
			/* Reset velocity (inertial impact) */
			for(ii = 0; ii < 3; ii++)
				pt->V[ii] = u0[ii];
		...
		if((Data_MeshType == CARTESIAN && TestOutsideDomain(pt->X)) || (Data_MeshType == UNSTRUCTURED && pt->ElementIndex < 0)) {
			...
			if(Trace_Compute) {
				...
				} /* If running exposure time computations */
				if(Trace_APCompute) {
					if(Data_MeshType == CARTESIAN)
