...
void OutputRT(void) {
	...
	if(Trace_GenerateMesh) { 
		...
		err = fread(&Trace_NumTracers, sizeof(int), 1, BinFileID);
		...
		err = fread(TraceIC_Array, sizeof(LagrangianPoint), Trace_NumTracers, BinFileID);
	...
	for(ss = 0; ss < Trace_NumLaunchTimes; ss++) {
		...
		ReadInTraceLaunch(ss, Trace_MeshPt);  
