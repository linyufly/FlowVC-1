...
void LoadUnstructVelDataFrame(int frame) {
	...
	if(Int_NormalFlow) {
		for(i = 0; i < Vel_SurfaceMeshNumNodes; i++) {
			...
			/* TEMP */
			/*
			 Vel_UnstructVelArray_U[slot2][Vel_SurfaceMeshNodeIDs[i]] = 0;
			 Vel_UnstructVelArray_V[slot2][Vel_SurfaceMeshNodeIDs[i]] = Int_TimeDirection * Int_NormalFlowScaling * 1;
			 Vel_UnstructVelArray_W[slot2][Vel_SurfaceMeshNodeIDs[i]] = 0;
			*/
		}
	}
...
void GetVelocity_Unstructured(const double tq, LagrangianPoint *pt, double *dXdt) {
	...
	tloc = (tq - Data_LoadedTMin) / (Data_LoadedTMax - Data_LoadedTMin);
	if((tloc > 1 + TINY) || (tloc < 0 - TINY)) 
		FatalError("tloc = %f and must be between 0 and 1 and tq = %f ", tloc,tq);
