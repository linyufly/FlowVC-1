...
void ReadInParameters(int argc, const char *argv[]) {
	...
	if(argc == 2) {
		...
		ReadInNextValue(Parameters_InFileID, &num_threads, 'd');
		...
		fprintf(stderr, "Number of Threads\t=\t%10d\n\n", num_threads);
...
void ReadInNextValue(FILE *Parameters_InFileID, void *pt, char type) {
	...
	while(1) {
		...
		if(c == '#') {
			...
			ssize_t r;
			r = fscanf(Parameters_InFileID, "%s %*s", buf);
			if(type == 's')
				r = fscanf(Parameters_InFileID,"%s\n", (char *) pt);
			else if(type == 'd')
				r = fscanf(Parameters_InFileID,"%d\n", (int *) pt);
			else if(type == 'f')
				r = fscanf(Parameters_InFileID,"%lf\n", (double *) pt);
			else 
...
void SetDerivedParameters(void) {
	...
	num_processors = omp_get_num_procs(); // Number of processors or (threads for hyperthreaded machine) on the machine
	printf("num_processors = %d \n",num_processors);
	
	if (FTLE_Compute) {
	
		if ((Output_TRes >= num_processors/2) || (!Int_Extrapolate)) {
			UPPERLOOP = 1;
			
			if((!Int_Extrapolate) && Output_TRes < num_processors) {
				fprintf(stderr,"Warning: Velocity Extrapolation required for multiple processes. FTLE calculation will be parallized according to the number of output files. \n \a");
	...
	if (Trace_Compute) {
		...
		if(Trace_NumLaunchTimes >= (num_processors/2)) {
			TRACER_UPPERLOOP = 1;
			fprintf(stderr, "Note: Tracer calculation will be parallized according to number of Tracer releases. \n \a");
		} else {
			TRACER_LOWERLOOP = 1;
			fprintf(stderr, "Note: Tracer Calculation will be parallized according to number of points to be integrated. \n \a");
		}
	}
...
void CheckParameters(void) {
	...
	if(Trace_Compute) {
		...
		if(Trace_ReleaseStrategy == STAGGERED) {
			if(Trace_AlwaysOutput)
				fprintf(stderr, "\nWarning: Trace_AlwaysOutput not supported for Trace_ReleaseStrategy = %d\n\n\a", STAGGERED);
